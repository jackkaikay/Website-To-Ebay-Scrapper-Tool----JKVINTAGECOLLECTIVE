...
headers = 'Item/Page Title, Image Code,Price,Listing Price, Free Shipping, size, Reccomended Size, Measurements,Colour, Item Url \n'
...
for link in links:
    try:
        ...
        if button.text != 'Sold Out':
            ...
            reccomended_size= paragraphs[1].text
            ...
            size = size.replace(',','|')
            reccomended_size = reccomended_size.replace(',','|')
            ...
            Imagecode = str(y)
            print(Imagecode)
            f.write(pagetitle + "," + str(Imagecode) + "," + price + "," + str(Listing_price_final) + "," + Freeshipping + "," + size + "," + reccomended_size + "," + Measurements + "," + Colour + "," + my_url + '\n')
